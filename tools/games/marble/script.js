...
class Labirynthe {
	...
	setupControls() {
		...
		if (this.isMobile) {
			...
			this.toile.addEventListener('touchstart', this.handleTouchStart.bind(this), false);
			this.toile.addEventListener('touchmove', this.handleTouchMove.bind(this), false);
			this.toile.addEventListener('touchend', this.handleTouchEnd.bind(this), false);
		}
	}
	handleOrientation(event) {
		const sensitivity = 0.05;
		this.force.x = event.gamma * sensitivity;
		this.force.y = event.beta * sensitivity;
	}
	...
	handleTouchMove(event) {
		...
		if (this.curseur.active) {
			...
			const maxForce = 100; // Adjust this value to change the maximum force
			const forceX = Math.min(Math.max(deltaX, -maxForce), maxForce) / maxForce;
			const forceY = Math.min(Math.max(deltaY, -maxForce), maxForce) / maxForce;
			
			this.force.x = forceX * 0.5; // Adjust the multiplier to change sensitivity
			this.force.y = forceY * 0.5;
		}
	}
