...
const blink = () => {
  setState("close"); // Set the state to "close" for the blink
  setTimeout(() => {
    setState(emotions[nextState()]); // Restore the previous emotion after the blink
  }, 500); // 0.5 seconds
};
...
setInterval(blink, Math.floor(Math.random() * 10000) + 3000); 
